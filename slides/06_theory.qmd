...
# Objectives
.align{center}
  .container borderwidth:{0.5pt} width:{60%} background:{.lightblue} radius:{4pt} padding:{0.5cm}
...
.row
  .container width:{60%}
    ...
    .align{center}
      .container padding:{0.5cm} borderwidth:{1pt} background:{.lightblue} alignment:{center}
  ...
  .container width:{40%}
    .container borderwidth:{1pt} padding:{0.5cm} background:{.lightred}
      ...
      **Special case:**
      ...
      $ q_n = 0 $ .ul{(no-flow) is very common for impermeable rock or clay layers}.
...
.row
  .container width:{50%}
    ...
    1. **Darcy velocity** $ q_n $ [L/T]
       - Flow per unit area
       - .ul{This is the source term we use the flow equation}
...
.align{center}
  .fullspan
    .container borderwidth:{1pt} padding:{0.5cm} radius:{10pt}
...
.row
  .container width:{100%}
    ...
    .row gap:{0.5cm}
      .container width:{33%} borderwidth:{1pt} padding:{0.5cm}
        **Dirichlet**
        - Large water bodies
...
.row gap:{1cm}
  .container width:{60%}
    !(100%)[Flownet](image/internet_flowlines.png)
  .container width:{40%}
...
.row gap:{1cm}
  .container width:{60%}
    !(100%)[Flow net under dam](image/franklin_fig_5-6.png "Seepage through permeable foundation")

  .container width:{40%}
    .container borderwidth:{1pt} padding:{0.5cm} background:{.lightblue}
      **Practical tip:**
      .fragment content:{Start with .ul{boundary conditions} from page before, then sketch equipotentials and streamlines iteratively until squares form.}
...
.row
  .container width:{50%}
    **For a single flow channel between two streamlines:** consider the 1d Darcy equation $ Q = A q = -A K \frac{\partial h}{\partial x} $
    ...
    .align{center}
      .container padding:{0.1cm} borderwidth:{1pt} background:{.lightblue} radius:{10pt}
  ...
  .container width:{50%}
    !(100%)[Homogeneous aquifer](image/franklin_fig_5-7.png "Streamlines and equipotentials under homogenous and isotropic conditions, flow is due to the constant heads applied at the left and right sides of the domain.")
