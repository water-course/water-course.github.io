...
# Objectives
.align{center}
  .container width:{60%} background:{.lightblue} radius:{4pt} padding:{0.5cm}
...
.row gap:{1cm} alignment:{center}
  .container width:{60%}
    ...
    ### **Good news**: At this introductory level, we don't need to solve these equations—just recognize and understand them.

  .container width:{40%}
    .container borderwidth:{1pt} padding:{0.5cm} background:{.lightred} alignment:{center}
      **Key Point**

      Hydrogeology provides simplified approaches for dealing with these equations.

# Recognizing Equation Features

.row gap:{1cm} alignment:{center}
  .container width:{55%}
    You don't need to be a math person for dealing with groundwater equations.Even as toddlers, we could recognize a rhinoceros by its features:
    - Four stubby legs
  ...
  .container width:{45%} alignment:{center} padding:{0.2cm} borderwidth:{0.5pt} background:{.lightblue} radius:{8pt}
...
.row gap:{1cm}
  .container width:{50%}
    ...
    .container borderwidth:{1pt} padding:{0.5cm} background:{.lightblue} radius:{10pt}
      **Three Essential Components:**
      1. **Governing equation** – describes groundwater flow
      2. **Domain** – geometry and layering
      3. **Boundary conditions** – head or flux at boundaries
...
.row gap:{1cm}
  ...
  .container width:{50%}
    !(100%)[Figure 5.1](image/franklin_fig_5-1.png)
...
.row
  .container width:{55%}
    **Case Study: Aquifer Between Two Rivers**
    ...
    .container borderwidth:{0.5pt} padding:{0.3cm} background:{.lightblue}
...
.align{center}
  .container width:{70%} borderwidth:{2pt} padding:{0.8cm} background:{.lightblue} alignment:{center}
...
.align{center}
  .container width:{70%} background:{.lightblue} padding:{0.5cm} alignment:{center} borderwidth:{0.5pt}
...
.row gap:{1cm}
  ...
  .container width:{40%}
    !(80%)[REV and flow directions](image/franklin_fig_5-3.png "Control volume")
...
.row
  .container width:{60%}
    ...
    **Inflow at left face:**
    ...
    .container width:{70%} padding:{0.3cm} borderwidth:{0.5pt} background:{.lightblue}
...
.align{center}
  .container padding:{0.5cm} borderwidth:{1pt} background:{.lightblue}
    **Net x-direction inflow:**
...
.row
  .container width:{50%}
    **Same logic applies to y and**
    ...
    .container padding:{0.3cm} borderwidth:{0.5pt} background:{.lightblue}
...
.align{center}
  .container width:{80%} padding:{0.5cm} borderwidth:{1pt} background:{.lightblue}
...
.row
  .container width:{60%}
    ...
    .container width:{90%} padding:{0.5cm} borderwidth:{1pt} background:{.lightblue}
...
.align{center}
  .container padding:{0.4cm} borderwidth:{2pt} background:{.lightblue}
...
.align{center}
  .container padding:{0.8cm} borderwidth:{2pt} background:{.lightblue}
...
.align{center}
  .container padding:{0.5cm} borderwidth:{1pt} background:{.lightblue} radius:{10pt}
...
.align{center}
  .container width:{85%} padding:{0.5cm} borderwidth:{1pt} background:{.lightblue}
...
.row
  ...
  .container width:{60%}
    !(100%)[Dupuit flow](image/franklin_fig_5-4.png "a) actual flow, b) Dupuit approximation")
...
.row
  .container width:{40%}
    **The flow equation alone is not enough!**
    ...
    .container borderwidth:{1pt} padding:{0.2cm} background:{.lightblue} radius:{10pt}
      **Three main types:**
      1. **Dirichlet** - specify head values
      2. **Neumann** - specify flow rates
      3. **Cauchy** - relate head and flow

  .container width:{60%}
    !(100%)[Boundary examples](image/franklin_fig_5-5.png)
    _Examples of different boundary condition types_
...
.row
  .container width:{60%}
    **What it specifies:** Hydraulic head values directly
    ...
    .align{center}
      .container padding:{0.2cm} borderwidth:{1pt} background:{.lightblue} radius:{10pt}
        $ h(x, y, z)|_\Gamma = h_1(x, y, z, t) $

    **Common examples:**
    - Large lakes or rivers (constant water level)
    - Ocean boundaries
    - Artificial ponds or reservoirs

  .container width:{40%}
    .container borderwidth:{1pt} padding:{0.5cm} background:{.lightred} radius:{10pt}
      **When to use:**
...
.row
  .container width:{60%}
    **What it specifies:** Water flux (flow rate) across the boundary
    ...
    .align{center}
      .container padding:{0.3cm} borderwidth:{1pt} background:{.lightblue}
        $ q_n|_\Gamma = q_n(x, y, z, t) $
...
.row
  ...
  .container width:{50%}
    ...
    2. **Volumetric flow rate** $ Q_n $ [L³/T]
...
# Type 3: Cauchy Boundary Conditions
...
.row
  .container width:{60%}
    **What it specifies:** Relationship between flux and head difference
    ...
    .align{center}
      .container padding:{0.3cm} borderwidth:{1pt} background:{.lightblue} radius:{10pt}
...
# Flow-Net Analysis: Visualizing Groundwater Flow

## From Equations to Pictures

.row
  .container width:{60%}
    Once we solve the flow equation, we can **visualize the results** using flow nets:
...
.row
  .container width:{60%}
    ...
    .container width:{80%} padding:{0.3cm} borderwidth:{1pt} background:{.lightblue}
...
.row
  .container width:{60%}
    **The square property is crucial for quantitative analysis:**

    - Makes flow calculations much simpler
    - Ensures proper flow distribution
    - Allows accurate flow estimation

    **Remember:** These aren't perfect squares, but "curvilinear squares" that maintain proper aspect ratios.

  .container width:{40%}
    .container borderwidth:{1pt} padding:{0.5cm} background:{.lightblue}
      **Practical tip:**

      Start with boundary conditions, then sketch equipotentials and streamlines iteratively until squares form.



# Flow Net Example: Dam Seepage
...
.row
  ...
  .container width:{45%}
    ...
    _Seepage through permeable foundation_
...
.row
  .container width:{60%}
    **For a single flow channel between two streamlines:**

    .container width:{85%} padding:{0.3cm} borderwidth:{1pt} background:{.lightblue}
  ...
  .container width:{40%}
    .container borderwidth:{1pt} padding:{0.5cm} background:{.lightred}
      **Key advantage:**
      ...
      For curvilinear squares, $ \Delta W / \Delta L ≈ 1 $, simplifying calculations.
